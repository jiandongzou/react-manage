...
export default function Index() {
    ...
    const showMenuModal = (flag, title) => {
        setIsMenuOpen(flag)
        setMenuModalTitle(title)
    }
    const showBtnAuthModal = (flag, title) => {
        setIsBtnAuthOpen(flag)
        setBtnAuthModalTitle(title)
    }

    const toggleMenuModal = (flag, values) => {
        setIsMenuOpen(flag)
        console.warn('表单内容', values)
    }
    const toggleBtnAuthModal = (flag, values) => {
        setIsBtnAuthOpen(flag)
        console.warn('表单内容', values)
    }

    async function getAuthBtn(path){
        setAuthLoading(true)
        const {data} = await getMenuBtnAuthList({ path })
        setBtnAuthData(data)
    ...
    async function getMenus() {
        ...
        const menu = data.map((item) => item.children[0])
        setMenuData(menu)
